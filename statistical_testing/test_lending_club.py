...
def statistical_testing_lending_club(shap_values_path, processed_data_path, random_state=42):
    print("Starting statistical testing for LendingClub dataset...")
    ...
    if isinstance(shap_values, list):
        shap_values_array = shap_values[1] # Assuming positive class explanations
    else:
    ...
    if np.any(valid_p_values_mask):
        ...
        for feature, res in corrected_results.items():
            if res["rejected"]:
                print(f"Feature \033[1m{feature}\033[0m: Corrected p={res['p_value_corrected']:.4f} (Significant)")
            # else:
    ...
    print("Statistical testing complete for LendingClub dataset.")

if __name__ == "__main__":
    processed_data_path = "explainable-credit-scoring/data_preprocessing/lending_club_processed_data_fold0.csv"
    shap_values_path = "explainable-credit-scoring/explanation_generation/lending_club_shap_values.pkl"
    statistical_testing_lending_club(shap_values_path, processed_data_path)
